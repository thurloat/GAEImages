...
def display_dollar_value(value):
  value = clean_dollar_value(value)
  if not value: return ''
  s = str(abs(value))
  if len(s) > 6: s =  s[:-6] + "," + s[-6:-3] + ',' + s[-3:]
  elif len(s) > 3: s = s[:-3] + ',' + s[-3:]
  return (value < 0 and '-' or '') + '$' + s

def display_percent_value(value):
  as_int = int(value+.00001)
  if abs(value - as_int) < .00001: return "%d%%" % as_int
  else: return "%f%%" % value

def display_per_thousand_value(value):
  as_int = int(value+.00001)
  if abs(value - as_int) < .00001: return "%d‰" % as_int
  else: return "%f‰" % value

def display_per_million_value(value):
  as_int = int(value+.00001)
  if abs(value - as_int) < .00001: return "%d‰‰" % as_int
  else: return "%f‰‰" % value
...
class DollarProperty(db.IntegerProperty):
  ...
  def get_value_for_form(self, instance):
    return display_dollar_value(getattr(instance, self.name))
...
class ListProperty(db.ListProperty):
  ...
  def __init__(self, item_type, verbose_name=None, default=None, **kwds):
    ...
    if issubclass(item_type, (db.Blob, db.Text)):
      ...
      kwds['indexed'] = True
    # self._require_parameter(kwds, 'required', True)  # THIS IS THE ONLY CHANGE FROM THE BUILT-IN ONE
    if default is None:
      default = []
  ...
  def validate_list_contents(self, value):
    for item in value:
      try:
        ...
        raise db.BadValueError('Items in the %s list must be castable to ints.  Problem was with %s' % (self.name, item))
    return [int(item) for item in value]

class DollarListProperty(IntegerListProperty):
  def get_value_for_form(self, instance):
    value = super(DollarListProperty, self).get_value_for_form(instance)
    if not value:
      return ""
    if isinstance(value, list):
      value = ', '.join(['$'+str(v) for v in value])
...
class Atlantic_tzinfo(datetime_module.tzinfo):
 """Implementation of the Atlantic timezone."""
 def utcoffset(self, dt):
   return datetime_module.timedelta(hours=-4) + self.dst(dt)
